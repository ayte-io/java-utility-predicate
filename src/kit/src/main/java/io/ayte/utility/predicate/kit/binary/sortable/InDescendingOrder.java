...
public class InDescendingOrder<T> implements AugmentedBinaryPredicate<T, T> {
    ...
    public static <T> BinaryPredicate<T, T> create(@NonNull Comparator<T> comparator) {
        return new InDescendingOrder<>(comparator);
    }

    public static <T extends Comparable<T>> BinaryPredicate<T, T> create() {
        return InDescendingOrder.<T>create(Comparator.naturalOrder());
    }
}
